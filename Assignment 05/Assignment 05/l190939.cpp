...
public:
	NetworkOptimiser() {
		...
		while (true) {
			...
			if (choice == 1) {
				...
				optimalGraph = this->optimizeNetwork();
	...
	// Save graph
	void saveGraph(const Graph& network) {
		...
		struct Connection {
			...
			Connection(int first = 0, int second = 0) {
				this->first = first;
				this->second = second;
			}
		...
		vector <Connection> stored;
		int totalConnections = 0;

		for (int i = 0; i < network.v; i++) {

			totalConnections += (network.adj[i].getSize() - 1);

		}

		fout << totalConnections / 2 << endl;

		for (int i = 0; i < network.v; i++) {

			auto j = network.adj[i].begin();
			j++;

			while (j != network.adj[i].end()) {

				Connection newC(j.curr->data.ID, j.curr->data.originID);

				if (find(stored.begin(), stored.end(), newC) == stored.end()) {
					
					stored.push_back(newC);
					swap(newC.first, newC.second);
					stored.push_back(newC);

					fout << newC.first << "," << newC.second << " " << j.curr->data.weight << endl;
	...
	Graph* optimizeNetwork() {
		...
		while (!minheap.empty()) {
			...
			if (minimum.originID != minimum.ID) {
				...
				originVertex.originID = minimum.ID;
